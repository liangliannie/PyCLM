...
def Plot_TimeSeries(obs,mod,site_id, col_num=0):
...
def Plot_TimeSeries_multiple_mod(obs, mmod, site_id, col_num=0):
    print('Process on TimeSeries ' + 'No.' + str(site_id) + '!')
    # print(obs.lat[site_id], obs.lon[site_id])
    x = np.ma.masked_invalid(obs.data[:, site_id])
    t = obs.time
    ...
    for i, mod in enumerate(mmod):
        ...
        yy = y[~x.mask]
        mods.append(yy)
        ax0.plot(t[~x.mask], yy, '-', label="Model " + str(i + 1), color=col[col_num])
...
def Plot_PDF_CDF_multiple_mod(obs,mmod,site_id, col_num=0):
    print('Process on PDF&CDF ' + 'No.' + str(site_id) + '!')
    x = np.ma.masked_invalid(obs.data[:, site_id])
    t = obs.time
...
def Plot_PDF_CDF_multiple_mod_seasonal(obs,mmod,site_id, col_num=0):
    print('Process on PDF&CDF ' + 'No.' + str(site_id) + '!')
    x = np.ma.masked_invalid(obs.data[:, site_id])
    t = obs.time
...
def Plot_Wavelet_multiple_mod(obs, obst, site_id, unit, model_name='Obs', col_num=0):
    print('Process on Wavelet ' + 'No.' + str(site_id) + '!')
    data = np.ma.masked_invalid(obs.data[:, site_id])
    time_data = obst[~data.mask]
...
def Plot_IMF_multiple_mod(obs, ot, mmod, mt, site_id, col_num=0):
...
def Plot_TimeSeries_cycle_multiple_mod(obs, mmod, site_id, cycle_length, col_num=0):
...
def Plot_TimeSeries_cycle_multiple_mod_season(obs, mmod, site_id, cycle_length, col_num=0):
...
def Plot_TimeSeries_cycle_reshape_multiple_mod(obs, mmod, site_id, cycle_length,xname="Hours of a day", col_num=0):
...
def Plot_TimeSeries_cycle_reshape_multiple_mod_season(obs, mmod, site_id, cycle_length,xname="Hours of a day", col_num=0):
...
def Plot_response4(obs1, mod1, obs2, mod2, obs3, mod3, obs4, mod4, site_id, col_num=0):
    ...
    plt.suptitle(obs4.name + ' (' + obs1.name + ', ' + obs2.name + ', ' + obs3.name + ') Response',
        fontsize=24)
...
def plot_4_variable_corr_multiple(obs1, mod1_1, obs2, mod2_1, obs3, mod3_1, obs4, mod4_1, site_id, col_num=0):
    print('Process on Corr4 ' + 'No.' + str(site_id) + '!')
    x1 = np.ma.masked_invalid(obs1.data[:, site_id])
    x1 = np.ma.masked_where(x1 == 0, x1)
    x2 = np.ma.masked_invalid(obs2.data[:, site_id])
...
mod1_1_1 = [mod11,mod12,mod13,mod14]
mod2_1_1 = [mod21,mod22,mod23,mod24]
mod3_1_1 = [mod31,mod32,mod33,mod34]
mod4_1_1 = [mod41,mod42,mod43,mod44]


lats = obs1.lat
lons = obs1.lon

for i, mname in enumerate(["mod1", "mod2"]):
    mod1_1 = [mod1_1_1[i]]
    mod2_1 = [mod2_1_1[i]]
    mod3_1 = [mod3_1_1[i]]
    mod4_1 = [mod4_1_1[i]]
    ...
    for siteid in range(len(lats)):
        ...
        fig0 = Plot_TimeSeries_multiple_mod(obs1, mod1_1, siteid, col_num=i)
        fig0.savefig(os.path.join(output_path, "%s_%s_timeseries_hourly.png" % (mname, region)), bbox_inches='tight')

        # fig7 = Plot_TimeSeries_cycle(obs1, mod1, siteid, 90.)
        fig71 = Plot_TimeSeries_cycle_multiple_mod(obs1, mod1_1, siteid, 1., col_num=i)
        fig71.savefig(os.path.join(output_path, "%s_%s_timeseries_daily.png" % (mname, region)), bbox_inches='tight')
        #
        # fig7 = Plot_TimeSeries_cycle(obs1, mod1, siteid, 90.)
        fig72 = Plot_TimeSeries_cycle_multiple_mod(obs1, mod1_1, siteid, 30., col_num=i)
        fig72.savefig(os.path.join(output_path, "%s_%s_timeseries_monthly.png" % (mname, region)),bbox_inches='tight')
        #
        # fig7 = Plot_TimeSeries_cycle(obs1, mod1, siteid, 90.)
        fig73 = Plot_TimeSeries_cycle_multiple_mod(obs1, mod1_1, siteid, 365., col_num=i)
        fig73.savefig(os.path.join(output_path, "%s_%s_timeseries_yearly.png" % (mname, region)),bbox_inches='tight')
        #
        #
        fig74 = Plot_TimeSeries_cycle_multiple_mod(obs1, mod1_1, siteid, 90., col_num=i)
        fig74.savefig(os.path.join(output_path, "%s_%s_timeseries_seasonly.png" % (mname, region)),bbox_inches='tight')

        # Cycle means output
        # fig8 = Plot_TimeSeries_cycle_reshape(obs1, mod1, siteid, 90.)
        fig8 = Plot_TimeSeries_cycle_reshape_multiple_mod(obs1, mod1_1, siteid, 1., col_num=i)
        fig8.savefig(os.path.join(output_path, "%s_%s_timeseries_hourofday.png" % (mname, region)),bbox_inches='tight')
        ...
        fig81 = Plot_TimeSeries_cycle_reshape_multiple_mod(obs1_2, mod1_2, siteid, 365., xname="Days of a year", col_num=i)
        fig81.savefig(os.path.join(output_path, "%s_%s_timeseries_dayofyear.png" % (mname, region)),bbox_inches='tight')
        ...
        for i, mod in enumerate(mod1_1):
            ...
            mod1_2.append(Variable(name=obs1.name,unit=obs1.unit,time=ot,data=np.mean(mdata, axis=1)))
        fig82 = Plot_TimeSeries_cycle_reshape_multiple_mod(obs1_2, mod1_2, siteid, 365., xname="Months of a year", col_num=i)
        fig82.savefig(os.path.join(output_path, "%s_%s_timeseries_monthofyear.png" % (mname, region)),bbox_inches='tight')
        ...
        for i, mod in enumerate(mod1_1):
            ...
            mod1_2.append(Variable(name=obs1.name,unit=obs1.unit,time=ot,data=np.mean(mdata, axis=1)))
        fig83 = Plot_TimeSeries_cycle_reshape_multiple_mod(obs1_2, mod1_2, siteid, 365., xname="Seasons of a year", col_num=i)
        fig83.savefig(os.path.join(output_path, "%s_%s_timeseries_seasonofyear.png" % (mname, region)),bbox_inches='tight')

        # PDF and CDF output
        # fig1 = Plot_PDF_CDF(obs1, mod1, siteid)
        fig1 = Plot_PDF_CDF_multiple_mod(obs1, mod1_1, siteid, col_num=i)
        fig1.savefig(os.path.join(output_path, "%s_%s_PDFCDF.png" % (mname, region)),bbox_inches='tight')
        ...
        fig2 = Plot_Wavelet_multiple_mod(np.mean(odata, axis=1), ot, siteid, obs1.unit, model_name='Obs', col_num=i)
        fig2.savefig(os.path.join(output_path, "%s_%s_wavelet.png" % (mname, region)), bbox_inches='tight')

        for i, mod in enumerate(mod1_1):
            mdata, mt, mtb = CycleReshape(mod, cycle_length=30.)
            fig2 = Plot_Wavelet_multiple_mod(np.mean(mdata, axis=1), mt, siteid, obs1.unit, model_name='Mod'+str(i), col_num=i)
            fig2.savefig(os.path.join(output_path, ("%s_%s_wavelet_Mod"+str(i) + ".png") % (mname, region)), bbox_inches='tight')
        ...
        fig3,fig4,fig5,fig6 = Plot_IMF_multiple_mod(np.mean(odata, axis=1), ot, mods, mt, siteid, col_num=i)

        fig3.savefig(os.path.join(output_path, "%s_%s_IMF1.png" % (mname, region)),bbox_inches='tight')
        fig4.savefig(os.path.join(output_path, "%s_%s_IMF2.png" % (mname, region)),bbox_inches='tight')
        fig5.savefig(os.path.join(output_path, "%s_%s_IMF3.png" % (mname, region)),bbox_inches='tight')
        fig6.savefig(os.path.join(output_path, "%s_%s_IMF4.png" % (mname, region)),bbox_inches='tight')

        fig9 = Plot_response2(obs1, mod1, obs2, mod2, siteid, col_num=i)
        fig9.savefig(os.path.join(output_path, "%s_%s_response.png" % (mname, region)),bbox_inches='tight')

        fig10 = Plot_response4(obs1, mod1, obs2, mod2, obs3, mod3, obs4, mod4, siteid, col_num=i)
        fig10.savefig(os.path.join(output_path, "%s_%s_response4.png" % (mname, region)),bbox_inches='tight')

        # fig11 = plot_4_variable_corr(obs1, mod1, obs2, mod2, obs3, mod3, obs4, mod4, siteid)
        fig11 = plot_4_variable_corr_multiple(obs1, mod1_1, obs2, mod2_1, obs3, mod3_1, obs4, mod4_1, siteid, col_num=i)
        fig11.savefig(os.path.join(output_path, "%s_%s_corr4.png" % (mname, region)),bbox_inches='tight')

        # GetSeasonMask(data_o, data_m, 0, 1)
        obs2_s1, mmod2_s1 = GetSeasonMask_multiplemod(obs1, mod1_1, siteid, 1)
        fig12 = Plot_TimeSeries_multiple_mod(obs2_s1, mmod2_s1, 0, col_num=i)
        fig12 = Plot_TimeSeries_cycle_multiple_mod_season(obs2_s1, mmod2_s1, 0, 365., col_num=i) # only one output is given
        fig12.savefig(os.path.join(output_path, "%s_%s_timeseries_s1.png" % (mname, region)), bbox_inches='tight')
        fig81 = Plot_TimeSeries_cycle_reshape_multiple_mod_season(obs2_s1, mmod2_s1, 0, 1., col_num=i)
        ...
        fig12 = Plot_TimeSeries_multiple_mod(obs2_s1, mmod2_s1, 0, col_num=i)
        fig12 = Plot_TimeSeries_cycle_multiple_mod_season(obs2_s2, mmod2_s2, 0, 365., col_num=i) # only one output is given
        fig12.savefig(os.path.join(output_path, "%s_%s_timeseries_s2.png" % (mname, region)), bbox_inches='tight')
        fig83 = Plot_TimeSeries_cycle_reshape_multiple_mod_season(obs2_s2, mmod2_s2, 0, 1., col_num=i)
        ...
        fig12 = Plot_TimeSeries_multiple_mod(obs2_s1, mmod2_s1, 0, col_num=i)
        fig12 = Plot_TimeSeries_cycle_multiple_mod_season(obs2_s3, mmod2_s3, 0, 365., col_num=i) # only one output is given
        fig12.savefig(os.path.join(output_path, "%s_%s_timeseries_s3.png" % (mname, region)), bbox_inches='tight')
        fig83 = Plot_TimeSeries_cycle_reshape_multiple_mod_season(obs2_s3, mmod2_s3, 0, 1., col_num=i)
        ...
        fig12 = Plot_TimeSeries_multiple_mod(obs2_s1, mmod2_s1, 0, col_num=i)
        fig12 = Plot_TimeSeries_cycle_multiple_mod_season(obs2_s4, mmod2_s4, 0, 365., col_num=i) # only one output is given
        fig12.savefig(os.path.join(output_path, "%s_%s_timeseries_s4.png" % (mname, region)), bbox_inches='tight')
        fig84 = Plot_TimeSeries_cycle_reshape_multiple_mod_season(obs2_s4, mmod2_s4, 0, 1., col_num=i)
