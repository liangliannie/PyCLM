...
def plot_imfs(signal, imfs, time_samples=None, fig=None, no=1, m=1, inityear=False):
    ...
    if inityear is not False:
        d_d_obs = np.asarray([str(inityear + int(x) / 365) + (
            '0' + str(int(x) % 365 / 31 + 1) if int(x) % 365 / 31 < 9 else str(int(x) % 365 / 31 + 1)) for x in
                              time_samples])
        ax.xaxis.set_ticks(
...
def plot_frequency(signal, imfs, time_samples=None, fig=None, no=1, m=1,inityear=False):
    ''' Author jaidevd https://github.com/jaidevd/pyhht/blob/dev/pyhht/visualization.py '''
    '''Original function from pyhht, but without plt.show()'''
    n_imfs = imfs.shape[0]
    # print(np.abs(imfs[:-1, :]))
...
def hht(data, imfs, time, no, fig3, freqsol=33, timesol=50, inityear=False):
